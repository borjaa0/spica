...
REGLAS:
- Un punto del orden del día es un asunto oficial de la sesión, anunciado como "el punto número X", "asunto número X", "primer punto", "punto único", etc.
- El número debe extraerse del texto cuando aparezca. "punto número 14" → numero: 14. "asunto número 3" → numero: 3. "primer punto" → numero: 1. No pongas null si el número está en el texto.
...
_PUNTO_NUM_RE = re.compile(
    r'(?:(?:punto|asunto)\s+n[uú]mero\s+' + _NUM_PAT_P + r'|' +
    r'\bprimer\s+punto\b|' +
    r'(?:punto|asunto)\s+' + _NUM_PAT_P + r'\b)',
    re.IGNORECASE,
)
...
def corregir_numero_punto(p: dict) -> dict:
    ...
    if n_llm is not None:
        # Si el LLM puso un número pero no hay ningún patrón punto/asunto en el texto → alucinación
        if m is None:
            p["numero"] = None
        return p
    # LLM no puso número → intentar extraer del texto
    if not m:
        return p
...
def main():
    ...
    _ANUNCIO_RE = re.compile(
        r'punto\s+n[uú]mero|punto\s+\d+|primer\s+punto|punto\s+[uú]nico|siguiente\s+punto'
        r'|asunto\s+n[uú]mero|asunto\s+\d+',
        re.IGNORECASE,
    )
    todos = [
        p for p in todos
