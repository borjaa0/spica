...
REGLA 1 — campo "punto":
- Si el texto dice "punto número X", "el punto X", "asunto número X", "punto único" → SIEMPRE es un punto del orden del día: "Punto X - [descripción breve]". Da igual que el contenido mencione una "propuesta relativa a..." o un ayuntamiento beneficiario: si lo introduce como "punto número X" es un Punto, no una Propuesta.
- Si el texto dice "punto de urgencia", "votar la urgencia", "urgencia" → etiqueta como "Urgencia - [descripción breve]". NUNCA le asignes un número inventado.
...
_SIES_RE = re.compile(_NUM_PAT + r'\s*s[ií](?:es?)?', re.IGNORECASE)
_CONTRA_RE = re.compile(_NUM_PAT + r'\s*(?:votos?\s+)?en contra', re.IGNORECASE)
_NOES_RE = re.compile(_NUM_PAT + r'\s*noes?', re.IGNORECASE)
...
def corregir_numeros(v: dict) -> dict:
    ...
    m = _FAVOR_RE.search(texto) or _SIES_RE.search(texto)
    v["favor"] = _parse_num(m.group(1)) if m else None
    m = _CONTRA_RE.search(texto) or _NOES_RE.search(texto)
